...
- **`running`** — still working. It exists for reconciliation, and baton records it without
  changing anything.
...
`baton/skill/SKILL.md` is the copy of these rules a worker reads. The launcher installs it in the
project at `.claude/skills/baton-worker/SKILL.md`.
...
the project to `running`. The worker records milestones along the way as `milestone` events.

When a worker reports `success`, baton logs a `lifecycle` event and a `phase` event moving to
`terminating`, waits the grace period so the worker can finish its own shutdown, sends `SIGTERM`
and then `SIGKILL` if the pane outlives the termination timeout, logs a `terminate` event, and
launches the next worker with the prompt the last one wrote — a `launch` event and a `phase` event
back to `running`.

A `completed` report ends the loop in phase `completed`. A `blocked` report moves the project to
phase `blocked` and leaves the worker alive for the human who must unblock it.
...
worker's `launch.sh`, which `exec`s `claude` with that id as its `--session-id`.
...
A `failed` report terminates the worker and stops the project in phase `failed`.
